...
boolean Ruan_is_focused=false;

float Ruan_Bandpower_DELTA;
float Ruan_Bandpower_THETA;

float Ruan_Bandpower_ALPHA;
float Ruan_Bandpower_BETA;
float Ruan_Bandpower_GAMMA;
...
////////////////////////////////////Ruan's Variables end/////////////////////////////


// Initialize bruanoards for constants
...
public void setup() {
    ...
    //////////////////// Ruan Server setup///////////////////////////////
    Ruan_server=new Server(this, 5204);
  //////////////////// Ruan Server setup end///////////////////////////////
...
public void initSystem() {
  ...
  if (eegDataSource == DATASOURCE_PLAYBACKFILE) {
    ...
    println("OpenBCI_GUI: initSystem: loading complete.  " + playbackData_table.getRowCount() + " rows of data, which is " + round(PApplet.parseFloat(playbackData_table.getRowCount())/getSampleRateSafe()) + " seconds of EEG data");
...
public void mouseOutOfBounds() {
  if (windowOriginSet && mouseInFrame) {
    ...
    if (mouseX > 0 && mouseX < width && mouseY > 0 && mouseY < height) {
      loc = getWindowLocation(JAVA2D);///P2D
...
class DataSourceBox {
  ...
  DataSourceBox(int _x, int _y, int _w, int _h, int _padding) {
    ...
    //sourceList.addItem(makeItem("SYNTHETIC (algorithmic)"));  // Ruan
    sourceList.addItem(makeItem("Example EEG"));  // RUAN
...
class DataProcessing {
  ...
  private int currentFilt_ind = 1;  // Ruan: set to 1 to default to 7-13Hz filter
  private int currentNotch_ind = 1;  // Ruan: set to 0 to default to 60Hz, set to 1 to default to 50Hz
  float data_std_uV[];
  float polarity[];
...
class TopNav {
  ...
  TopNav(){
    ...
    int butNum = 1;
    tutorialsButton = new Button(width - 3*(butNum) - 80, 3, 80, 3, "", fontInfo.buttonLabel_size);
    ...
    butNum = 2;
    issuesButton = new Button(width - 3*(butNum) - 80 - tutorialsButton.but_dx, 3, 80, 3, "", fontInfo.buttonLabel_size);
    ...
    butNum = 3;
    shopButton = new Button(width - 3*(butNum) - 80 - issuesButton.but_dx - tutorialsButton.but_dx, 3, 80, 3, "", fontInfo.buttonLabel_size);
...
class W_BandPower extends Widget {
  ...
  public void draw(){
    ...
    plot3.endDraw();
     ...
     Ruan_Bandpower_DELTA=dataProcessing.headWidePower[DELTA];
     Ruan_Bandpower_THETA=dataProcessing.headWidePower[THETA];
  
     Ruan_Bandpower_ALPHA=dataProcessing.headWidePower[ALPHA];
     Ruan_Bandpower_BETA=dataProcessing.headWidePower[BETA];
     Ruan_Bandpower_GAMMA=dataProcessing.headWidePower[GAMMA];
     ////////////////////////////Ruan End////////////////////////////////
  ...
  public void draw() {
    ...
    for (int i = 0; i < rowNum; i++) {
      for (int j = 0; j < colNum; j++) {
        ...
        ellipse(2*colOffset/8, rowOffset / 2, scaleFactor * motorWidgets[i * colNum + j].myAverage, scaleFactor * motorWidgets[i * colNum + j].myAverage);
        //////////////////////////////////////////////////Ruan////////////////////////////////////////////////
        Ruan_EMG[i * colNum + j]= motorWidgets[i * colNum + j].myAverage;
  ...
  public void process(float[][] data_newest_uV, //holds raw EEG data that is new since the last call
    ...
    for (int Ichan=0; Ichan < nchan; Ichan++) {
      ...
      for (int Ibin=0; Ibin < fftBuff[Ichan].specSize(); Ibin++) {
        ...
        //println("from EMG EEG_User: Ichan="+Ichan+",Freq="+FFT_freq_Hz+"Hz.FFT Value["+FFT_value_uV+"uv/bin");// By ruan
      }
    }
    //---------------------------------------------------------------------------------
  }
...
//      for (int Ichan=0; Ichan < nchan; Ichan++){   // Ruan: Consider all 8 channels.
    ...
    ///////////////////////////  Ruan///////////////////////////////////////////////
      ...
      Ruan_is_focused=isFocused;
      
 Ruan_focus_value=abs(alpha_avg-(alpha_upper+alpha_thresh)/2)+abs(beta_avg-(beta_upper+beta_thresh)/2);  // 0 is the best.
     ...
     /* if (isFocused == true){
      ...
      else
      {
         ...
         //alpha_thresh=alpha_thresh+0.01;
         //alpha_upper=alpha_upper+0.01;
         //beta_thresh=beta_thresh+0.01;
      }*/
        
   // Ruan_focus_value=1/(abs(alpha_avg/((alpha_upper+alpha_thresh)/2))+abs(beta_avg/(beta_upper+beta_thresh)/2));
    //////////////////////////////////////////////////////////////////////////////////////  
...
//// Put new widget with one by on turn.
//////////////Set ruan Widget as the first Widget.////////////////
  w_template1 = new W_template(_this);
  w_template1.setTitle("Sending EEG Data");  //Ruan Code
...
class WidgetManager{
  ...
  WidgetManager(PApplet _this){
    ...
    if(nchan == 4 && eegDataSource == DATASOURCE_GANGLION){
      ...
      currentContainerLayout = 11; //Ruan: very important!!!! setup the initial layout here! 11 is the last layout type.  default layout ... tall container left and 2 shorter containers stacked on the right
      setNewContainerLayout(currentContainerLayout); //sets and fills layout with widgets in order of widget index, to reorganize widget index, reorder the creation in setupWidgets()
    }
...
class Layout{
  ...
  public void settings() {  size(1024, 768,JAVA2D);  smooth(); }
